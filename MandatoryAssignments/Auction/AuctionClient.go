...
func (client *AuctionClient) connectToServer() {
	// Connect to a valid server using the specified criteria
	conn := client.findValidServerConn()
...
func (client *AuctionClient) findValidServerConn() (connection *grpc.ClientConn) {
	...
	for i := 0; ; i++ {
		...
		if invalidPortsInARow >= 10 {
			// If 10 consecutive invalid ports are found, break out of the loop
			break
		}
	...
	if connection == nil {
		...
		log.Printf("Client %v: No leader found. Forcing election on first server found\n", client.ID)
		...
		for i := 0; ; i++ {
			...
			if invalidPortsInARow >= 10 {
				...
				log.Printf("Client %v: Failed to find any server\n", client.ID)
				return nil
			}
		...
		// Call for election on found server
		_, err := s.NewAuctionClient(connection).CallElection(context.Background(), &s.ID{ID: 99999})
		if err != nil {
		...
		return client.findValidServerConn()
	}
...
func (client *AuctionClient) Bid(amount int32) {
	...
	if !ack.Valid {
		...
		case 1:
			response = "Another server is the leader. Reconnecting in 5 seconds and retrying bid..."
			time.Sleep(5 * time.Second)
			client.connectToServer()
			client.Bid(amount)
			return
...
func (client *AuctionClient) Update() {
	...
	if err != nil {
		// Attempt to reconnect to the server and return
		log.Printf("Client %v: Lost connection to server. Attempting to reconnect to server in 5 seconds...\n", client.ID)
		fmt.Printf("Attempting to reconnect to server in 5 seconds...\n")
		time.Sleep(5 * time.Second)
		client.connectToServer()
		return
	}
